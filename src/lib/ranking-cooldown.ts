export const RANKING_OPT_IN_COOLDOWN_MS = 24 * 60 * 60 * 1000;

export function getRankingOptInCooldown(
  ...
  const nextChangeAt = new Date(changedAt.getTime() + RANKING_OPT_IN_COOLDOWN_MS);
...
export function formatRankingCooldownRemaining(remainingMs: number): string {
